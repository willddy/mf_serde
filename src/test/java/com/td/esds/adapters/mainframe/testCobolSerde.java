...
//	}
	...
	public void testDeserializeWritable() throws SerDeException {
		tbl.setProperty("cobol.layout.literal", "01 WS-VAR. 05 WS-NAME PIC X(12). 05 WS-MARKS-LENGTH PIC 9(3). 05 WS-MARK OCCURS 0 to 25 TIMES DEPENDING ON WS-MARKS-LENGTH. 10 WS-MARK PIC 999. 05 WS-NICKNAME PIC X(6).");
		csd.initialize(conf, tbl);
		System.out.println(csd.deserialize(new Text("002123123")));
		...
		tbl.setProperty("cobol.layout.literal", "01 WS-VAR. 02 WS-NAME PIC X(12). 02 W-Ram. 05 WS-MARKS-LENGTH PIC 9(3). 02 ws-kanu. 03 ws-lipi. 05 WS-marks PIC 9(3). 05 WS-MARK PIC 999. 05 WS-NICKNAME PIC X(6)");
		csd.initialize(conf, tbl);
